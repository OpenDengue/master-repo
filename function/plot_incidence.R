  ...
  ggplot()+
   #geom_col(aes(fill=factor(source_cat)), color="grey50", linewidth=0.05)+
    geom_bar(data=x, stat="identity", 
             aes(x=epidemiological_week, y=dengue_na_approx_inc, fill=source_cat)) +
   geom_point(data=x, 
             aes(x=epidemiological_week, y=dengue_na_approx_inc, color = source_cat))+
...
plot_incidence_spline<- function(x){
 # create a list of all possible group categories for all countries
all_source <- c("Raw", "Imputed", "Zero")
...
names(colors) <- all_source  # assign names to each color
  ...
  ggplot()+
   #geom_col(aes(fill=factor(source_cat)), color="grey50", linewidth=0.05)+
    geom_bar(data=x, stat="identity", 
             aes(x=epidemiological_week, y=dengue_na_spline_inc, fill=source_cat)) +
   geom_point(data=x, 
             aes(x=epidemiological_week, y=dengue_na_spline_inc, color = source_cat))+
    scale_fill_manual(values=colors, 
                     breaks=c("Raw", "Imputed", "Zero"), 
                     labels=c("Raw", "Imputed", "Zero")) +
    scale_color_manual(values=colors, 
