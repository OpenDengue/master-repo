setwd("C:/Users/AhyoungLim/Dropbox/WORK/Dengue Project 2022/Dengue DB/master-repo")
...
showtext_auto(); font_add_google("Open Sans")

# DATA ====================================================
S_data <- read.csv("C:/Users/AhyoungLim/Dropbox/WORK/Dengue Project 2022/11_DENData/01_Dengue_data/OD_master/OD_V1.2/releases/Spatial_extract_V1_2.csv")
T_data <- read.csv("C:/Users/AhyoungLim/Dropbox/WORK/Dengue Project 2022/11_DENData/01_Dengue_data/OD_master/OD_V1.2/releases/Temporal_extract_V1_2.csv") 
...
temp_boolean <- T_data %>%
  ...
  slice_head(n=1) %>% select(-n)%>% ungroup()%>%
...
# exclude for now
dt_heatmap<- dt_heatmap%>% filter(!adm_0_name %in% c("Yemen", "Sudan", "Mongolia"))
...
dt_heatmap$subregion <- ifelse(dt_heatmap$adm_0_name %in% americas1, "North & Central America", 
                          ifelse(dt_heatmap$adm_0_name %in% americas2, "South America", 
                            ifelse(dt_heatmap$adm_0_name %in% americas3, "Caribbean", NA)))


dt_heatmap$subregion[dt_heatmap$region == "Asia"] <- ifelse(dt_heatmap$adm_0_name[dt_heatmap$region == "Asia"] %in% asia1, "Southeast Asia", 
                          ifelse(dt_heatmap$adm_0_name[dt_heatmap$region == "Asia"] %in% asia2, "South Asia", 
                            ifelse(dt_heatmap$adm_0_name[dt_heatmap$region == "Asia"] %in% asia3, "East Asia", 
                             ifelse(dt_heatmap$adm_0_name[dt_heatmap$region == "Asia"] %in% asia4, "Oceania", NA))))
...
dt_heatmap$subregion <- factor(dt_heatmap$subregion, levels = c("North & Central America", 
                                                                "South America", 
                                                                "Caribbean", 
                                                                "Southeast Asia", 
                                                                "South Asia", 
...
heatmap_base <- function(data, region_name, type, subregion = FALSE) { 
  scale_temporal <-   scale_fill_manual(name = "Temporal resolution",
                    values = c("#479A5A", "#99D8CA", "#E5F5F9"), na.value="#D9D9D9")
  scale_spatial <-    scale_fill_manual(name = "Spatial resolution",
  ...
  ggplot(aes(x=Year, y=factor(adm_0_name, level=country_order), fill=!!sym(fill_var), 
...
p <- subplot(style(p1, showlegend=F), p2, nrows=1, margin = 0.08 ) %>% 
  layout(autosize=T,  margin = list(t = 40),
         legend = list(orientation = "h",   # show entries horizontally
                     xanchor = "center",  # use center of legend as anchor
                     x = 0.5, y=-0.05), 
         title =  "Best temporal resolution available")

saveWidget(p, "docs/figure/heatmap_temporal_.html", selfcontained = T, libdir = "lib")
...
combined <- combined + plot_layout(guides = "collect")+
    plot_annotation(title = "Best temporal resolution available", 
                    theme = theme(plot.title = element_text(size = 30, face="bold", 
                                                            family = "Open Sans", hjust = 0.5)))


ggsave(paste0("docs/figure/heatmap_temporal.png"), combined, height = 6.4, width = 10.8, dpi=300, bg = "white" )
...
saveWidget(p, "docs/figure/heatmap_spatial_.html", selfcontained = T, libdir = "lib")
...
combined2 <- combined2 + plot_layout(guides = "collect")+
    plot_annotation(title = "Best spatial resolution available", 
                    theme = theme(plot.title = element_text(size = 30, face="bold", 
                                                            family = "Open Sans", hjust = 0.5)))
ggsave(filename = "docs/figure/heatmap_spatial.png", combined2, height = 6.4, width = 10.8, units="in", dpi=300, bg = "white" )
