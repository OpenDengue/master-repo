...
Take a look at the visual summaries of our [data coverage](./docs/data) and if you are aware of a data source for a country or time period that we have not included please get in touch. The data source must be publicly accessible (e.g. on a ministry of health website) and denominator based (e.g. 10 cases from a defined population over a defined period of time) but can be in any format (graph, table, etc) and any language. Please see our [Methods](./docs/overview) page for further details on the types of data we can and cannot include.
